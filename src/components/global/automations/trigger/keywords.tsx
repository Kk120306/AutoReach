...
const Keywords = ({ id }: KeywordsProps) => {
    ...
    return (
        <div className="bg-background-80 flex flex-col gap-y-3 p-3 rounded-lg">
            ...
            <div className="flex flex-wrap justify-start gap-2 items-center">
                {data?.data?.keywords && data?.data?.keywords.length > 0 &&
                    data?.data?.keywords.map((word) =>
                        word.id !== latestVariable.variables.id && (
                            <div
                                key={word.id}
                                className="bg-background-90 flex items-center gap-x-2 capitalize text-text-secondary py-1 px-4 rounded-lg"
                            >
                                <p>{word.word}</p>
                                <X
                                    size={20}
                ...
                {latestVariable && latestVariable.status === 'pending' && (
                    <div className="bg-background-90 flex items-center gap-x-2 capitalize text-text-secondary py-1 px-4 rounded-lg">
