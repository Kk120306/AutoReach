...
const AutomationBreadCrumb = ({ id }: AutomationBreadCrumbProps) => {
    ...
    return (
        <div className="rounded-lg w-full p-5 bg-[#18181B1A] flex items-center">
            <div className="flex items-center gap-x-2 min-w-0">
                <p className="text-[#9B9CA0] truncate">Automations</p>
                <ChevronRight className="flex-shrink-0 text-[#9B9CA0]" />
                <span className="flex gap-x-3 items-center min-w-0">
                    {edit ? (
                        <Input ref={inputRef}
                            placeholder={isPending ? latestVariable.variables : "Create a new name"}
                            className="bg-transparent h0auto outline-none text-base border-none p-0" />
                    ) : (
                    ...
                    {edit ? (
                        ...
                        <span className="cursor-pointer hover:opactity-75 duration-100 transition flex-shrink-0 mr-4"
                            onClick={enableEdit}
                        >
                            <PencilIcon size={14} />
                        </span>
